...
=begin old code
  module SplittableString
    ...
    # def to_sentence(); end
    # def to_line();     end
    # def count_visible_char();   end # \s=0, CRLF=0
    # def count_printable_char(); end # \s=0, CRLF=0
    def count_logical_char();   end # \s=0, CRLF=0
    def count_physical_char();  end # \s=1, CRLF=1
    def count_byte();           end
    def count_word();           end
    # def count_block();    end
    # def count_sentence(); end

    def count_line(); end  # last line may include no eol.
    def count_eol();  end
...
if $0 == __FILE__
  ...
  class String
    include DocDiff::SplittableString
  end

  if ARGV.size > 0
    str = ARGF.read
  else
    str = String.new("Foo, bar, \nBaz.  \n´Á»ú¡¢\n¤«¤Ê¡£\n")
  end

  erroneous = "CR\rLF\nCRLF\r\nnoeol"
  sunix = NKF.nkf('-e', str).gsub(/\n/, "\n") + erroneous
  smac  = NKF.nkf('-s', str).gsub(/\n/, "\r") + erroneous
  sdos  = NKF.nkf('-s', str).gsub(/\n/, "\r\n") + erroneous

  puts "English, ASCII, CR"
  s = smac.dup
  s.lang, s.enc, s.eol = "English", "ASCII", "\r"
  puts s.to_char, s.to_word, s.to_line

  puts "Japanese, EUC-JP, LF"
  s = sunix.dup
  s.lang, s.enc, s.eol = "Japanese", "EUC-JP", "\n"
  puts s.to_char, s.to_word, s.to_line

  puts "Japanese, Shift_JIS, CRLF"
  s = sdos.dup
  s.lang, s.enc, s.eol = "Japanese", "Shift_JIS", "\r\n"
  puts s.to_char, s.to_word, s.to_line
